...
export default function ModifyFreeResourceCategoryModal({
  ...
  return (
    <>
      <Modal open={open} onClose={handleClose} size="full" title="Modify Free Resource Category" showCloseButton={false}>
        <div className="overflow-hidden rounded-xl bg-[#f7f7f7] shadow-[0_24px_60px_rgba(15,23,42,0.2)]">
          <ModalPanelHeader
            title="Modify Free Resource Category"
            onClose={handleClose}
            icon={Layers}
            closeVariant="icon"
          />
      ...
      <ConfirmDeleteDialog
        ...
        message={
          deleteTarget?.ids?.length > 1
            ? `Delete ${deleteTarget.ids.length} categories? Resources using them will be marked Uncategorized.`
            : `Delete "${deleteTarget?.name || 'this category'}"? Resources using it will be marked Uncategorized.`
        }
