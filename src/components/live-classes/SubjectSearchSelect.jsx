      ...
      {open && (
        ...
        >
          ...
          <ul className="custom-scrollbar max-h-56 overflow-y-auto py-1">
            {error && <li className="px-4 py-3 text-sm text-[#c96565]">{error}</li>}
            {!error && filtered.length === 0 && (
              <li className="px-4 py-3 text-sm text-[#9ca0a8]">No subjects found</li>
            )}
            {filtered.map((s) => (
              <li key={s.id}>
                <button
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block font-semibold text-[#222]">{s.name}</span>
                    <span className="block truncate text-xs text-[#686868]">
                      {s.mainCategoryName}
                    </span>
                  </span>
                  {value === s.id && <Check className="h-4 w-4 shrink-0 text-[#55ace7]" />}
                </button>
              </li>
            ))}
          </ul>
