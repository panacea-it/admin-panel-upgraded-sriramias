...
export default function BatchSearchSelect({
  ...
  return (
    <div className={cn('relative', className)} ref={rootRef} onKeyDown={onKeyDown}>
      ...
      {open && (
        <div
          className="absolute z-[130] mt-2 w-full overflow-hidden rounded-xl border border-[#e8f4fc] bg-white shadow-[0_16px_40px_rgba(36,99,146,0.15)]"
          role="listbox"
          id={listboxId}
        >
                ...
                <li key={opt.id}>
                  <button
                    ...
                    className={cn(
                      ...
                      index === highlight && opt.selectable && 'bg-[#e8f4fc]',
                    )}
                  >
                    <span className="min-w-0 flex-1">
                      <span className="block font-semibold text-[#222]">{opt.batchName}</span>
                      <span className="block font-mono text-[11px] text-[#246392]">{opt.batchId}</span>
